...
public class PasswordSigninService(
    ...
    IValidator<PasswordSigninDto> validator,
    ISessionControlService sessionControlService)
{
    public async Task<SigninResponseDto> SigninAsync(PasswordSigninDto dto)
    {
    ...
    private void InitClient(PasswordSigninDto dto)
    {
        if (dto.Client == default)
            dto.Client = detector.GetClientInfo().MapToClientInfo();
    }

    private async Task<SigninResponseDto> SigninMfaAsync(IdentityUser user, PasswordSigninDto dto, IdentityClient client, LocationInfo location)
    {
    ...
    private async Task<SigninResponseDto> SigninAsync(IdentityUser user, PasswordSigninDto dto, IdentityClient client, LocationInfo location)
    {
        var sessionId = Guid.CreateVersion7();
